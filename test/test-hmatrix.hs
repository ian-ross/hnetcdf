...
hmatrixDataOrdering = do
  let xdim = NcDim "x" 5 False
      ...
      z1 = matrix 5 [1..50]    :: Matrix Double
      z1C = cmap realToFrac z1 :: Matrix CDouble
      z2 = fromColumns $ toColumns z1 :: Matrix Double
      z2C = cmap realToFrac z2        :: Matrix CDouble
  let write nc = do
        ...
        put nc z1var $ HMatrix z1C
        put nc z2var $ HMatrix z2C -- XXX fails
        return ()
  withCreateFile ncout write (putStrLn . ("ERROR: " ++) . show)
  let read nc = do
        let z1var = fromJust $ ncVar nc "z1"
            z2var = fromJust $ ncVar nc "z2"
        Right (HMatrix z1C) <- get nc z1var :: HMatrixRet CDouble
        Right (HMatrix z2C) <- get nc z2var :: HMatrixRet CDouble
        let z1 = cmap realToFrac z1C :: Matrix Double
            z2 = cmap realToFrac z2C :: Matrix Double
        return (z1, z2)
  (z1tst, z2tst) <-
